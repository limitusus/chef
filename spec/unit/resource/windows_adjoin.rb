...
describe Chef::Resource::WindowsAdJoin do
  ...
  it "supports :join and :set_dns_server actions" do
    expect { resource.action :join }.not_to raise_error
    expect { resource.action :set_dns_server }.not_to raise_error
    expect { resource.action :leave }.to raise_error(ArgumentError)
  end
end
